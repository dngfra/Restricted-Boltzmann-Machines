...
class Encoder(tf.keras.layers.Layer):
    ...
    def call(self, input_features):
        activation = self.hidden_layer_5(self.hidden_layer_4(self.hidden_layer_3(self.hidden_layer_2(self.hidden_layer_1(input_features)))))
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, code):
        activation = self.hidden_layer_5(self.hidden_layer_4(self.hidden_layer_3(self.hidden_layer_2(self.hidden_layer_1(code)))))
        return self.output_layer(activation)
...
if __name__ == '__main__':
    ...
    autoencoder.fit(x_train_binary, x_train_binary , epochs=100,batch_size=64,validation_data = (x_test_binary,x_test_binary), callbacks = [cp_callback])
    autoencoder.save_weights('auto_weights_tanh.h5')

    test = np.zeros((50, 784))
    for i in range(10):
        mask = np.where(y_train == i)
        ciao = x_train_binary[mask]
        ciao2 = ciao[:5]
        test[i * 5:i * 5 + 5] = ciao2

    indx = np.random.randint(0, x_test_binary.shape[0], 20)
    ...
    for en in range(50):
        ...
        axes[en][0].imshow(test[en, :].reshape(28, 28), cmap='Greys')
        axes[en][0].set_title("Original Image")
        axes[en][1].imshow(np.asarray(reconstruction_plot).reshape(28, 28), cmap='Greys')
        axes[en][1].set_title("Reconstruction")
        axes[en][2].bar(x, encoded.numpy().reshape((10)))
...
def loss(model, original):
    reconstruction_error = tf.multiply(original,
                       tf.where(tf.math.is_inf(tf.math.log(model(original))), np.zeros_like(tf.math.log(model(original))), tf.math.log(model(original)))) \
           + tf.multiply((1 - original),
                         tf.where(tf.math.is_inf(tf.math.log(1 - model(original))), np.zeros_like(tf.math.log(1 - model(original))),
                                  tf.math.log(1 - model(original))))
    return -tf.reduce_sum(reconstruction_error,1)[0]


def train(loss, model, opt, original):
    with tf.GradientTape() as tape:
        gradients = tape.gradient(loss(original,model), model.trainable_variables)
...
with writer.as_default():
    with tf.summary.record_if(True):
        for epoch in range(epochs):
            ...
            rnd_test_points_idx = np.random.randint(low=0, high=x_test_binary.shape[0], size = 1)[0]
            loss_values = loss(x_test_binary[rnd_test_points_idx],autoencoder)
            original = tf.reshape(batch_features, (batch_features.shape[0], 28, 28, 1))
            reconstructed = binarizer.fit_transform(tf.reshape(autoencoder(tf.constant(batch_features)),
                                       (batch_features.shape[0], 28, 28, 1)))
            tf.summary.scalar('loss', loss_values, step=epoch)
            tf.summary.image('original', original, max_outputs=10, step=epoch)
            tf.summary.image('reconstructed', reconstructed, max_outputs=10, step=epoch)
'''
