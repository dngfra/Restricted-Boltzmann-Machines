...
class CVAE(tf.keras.Model):
  def __init__(self, latent_dim,num_classes,f_in=3,f_out=3):
    ...
    self.inference_net = tf.keras.Sequential(
        [
          tf.keras.layers.Conv2D(32, (f_in, f_in), activation='relu', padding="same", input_shape=(32, 32, 1)),
          tf.keras.layers.BatchNormalization(),
          tf.keras.layers.MaxPooling2D((2, 2)),
          tf.keras.layers.Dropout(0.5),
          tf.keras.layers.Conv2D(64, (f_in, f_in), padding="same", activation='relu'),
          tf.keras.layers.BatchNormalization(),
          tf.keras.layers.MaxPooling2D((2, 2)),
          tf.keras.layers.Dropout(0.5),
          tf.keras.layers.Conv2D(64, (f_in, f_in), padding="same", activation='relu'),
    ...
    self.generative_net = tf.keras.Sequential(
        [
          ...
          tf.keras.layers.Conv2DTranspose(filters=64,kernel_size=f_out,strides=(1, 1),padding="SAME",activation='relu'),
          tf.keras.layers.UpSampling2D(size=(2, 2)),
          tf.keras.layers.Conv2DTranspose(filters=64,kernel_size=f_out,strides=(1, 1),padding="SAME",activation='relu'),
          tf.keras.layers.UpSampling2D(size=(2, 2)),
          tf.keras.layers.Conv2DTranspose(filters=32,kernel_size=f_out,strides=(1, 1),padding="SAME",activation='relu'),
          tf.keras.layers.Conv2DTranspose(filters=1, kernel_size=f_out, strides=(1, 1), padding="SAME"),
        ]
    )
...
if __name__ == '__main__':
    ...
    class_names = [keys[i] for i in [11]]
    print(class_names)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 5 == 0 or epoch == 1:
            ...
            #data = tf.concat([data_bin[class_names[0]][i],data_bin[class_names[1]][i],data_bin[class_names[2]][i],data_bin[class_names[3]][i],data_bin[class_names[4]][i],data_bin[class_names[5]][i]],0)
